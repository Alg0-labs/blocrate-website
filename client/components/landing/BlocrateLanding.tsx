// @ts-nocheck
/**
 * Blocrate landing — a faithful React port of website/index.html.
 * Markup mirrors the original DOM (see /website/index.html); the bespoke CSS is
 * reused verbatim, scoped under `.bl-root` (client/landing.css); the vanilla-JS
 * animations are ported into the effect below. Assets (phone render, wordmarks,
 * protocol icons) are the originals, extracted to assets/landing/assets.json.
 */
import { useEffect, useRef } from "react";
import "../../landing.css";
import A from "@/assets/landing/assets.json";
...
export default function BlocrateLanding() {
  ...
  useEffect(() => {
    ...
    // ===== HERO: brand logo, flip card cycle, floating constellation =====
    if ($("brandLogo")) $("brandLogo").src = A.blocrate_black;

    (function () {
      const icons = [A.aave, A.morpho, A.maple];
      const front = $("faceFront"), back = $("faceBack"), card = $("flipCard"), stage = $("flipStage");
      if (!front || !back || !card || !stage) return;
      let idx = 0, flipped = false;
      front.src = icons[0]; back.src = icons[1 % icons.length];
      function tick() {
        idx = (idx + 1) % icons.length;
        const next = icons[idx];
        if (flipped) front.src = next; else back.src = next;
        flipped = !flipped;
        card.style.transform = flipped ? "rotateY(180deg)" : "rotateY(0deg)";
        const t = setTimeout(() => {
          stage.classList.add("settle");
          const t2 = setTimeout(() => stage.classList.remove("settle"), 620);
          cleanups.push(() => clearTimeout(t2));
        }, 760);
        cleanups.push(() => clearTimeout(t));
      }
      if (!reduce) {
        const s = setTimeout(() => { const iv = setInterval(tick, 2200); cleanups.push(() => clearInterval(iv)); tick(); }, 1100);
        cleanups.push(() => clearTimeout(s));
      }
    })();
    ...
    (function () {
      ...
      FIELD.forEach((it, i) => {
        ...
        el.style.width = it.s + "px"; el.style.height = it.s + "px";
        el.style.setProperty("--dur", it.dur + "s");
        el.style.setProperty("--tx", it.tx);
        el.style.setProperty("--ty", it.ty);
        el.style.animationDelay = (0.1 + i * 0.06) + "s" + (reduce ? "" : ", " + (i * 0.4) + "s");
        const img = document.createElement("img"); img.src = A[it.k]; img.alt = "";
        el.appendChild(img); field.appendChild(el); els.push(el);
      });
      if (!reduce && window.matchMedia("(pointer:fine)").matches) {
        const mm = (e) => {
          const cx = (e.clientX / window.innerWidth - 0.5), cy = (e.clientY / window.innerHeight - 0.5);
          els.forEach((el, i) => { const d = FIELD[i].lvl * 5; el.style.marginLeft = (cx * d) + "px"; el.style.marginTop = (cy * d) + "px"; });
        };
    ...
    // ===== S1: asset wiring =====
    if ($("frameimg")) $("frameimg").src = A.phone;
    if ($("modalwm")) $("modalwm").src = A.wm_white;
    if ($("cardlogo")) $("cardlogo").src = A.wm_white;
    if ($("lg1")) $("lg1").innerHTML = '<img src="' + A.aave + '"/>';
    if ($("lg2")) $("lg2").innerHTML = '<img src="' + A.morpho + '"/>';
    if ($("lg3")) $("lg3").innerHTML = `<div style="width:100%;height:100%;background:linear-gradient(135deg,#5b8cff,#3d5cff);display:flex;align-items:center;justify-content:center"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2"><rect x="2" y="6" width="20" height="13" rx="2.5"/><line x1="2" y1="10.5" x2="22" y2="10.5"/></svg></div>`;

    // ===== S1: reading reveal + highlighter =====
    (function () {
      const COPY = [
      ...
      COPY.forEach((line, li) => {
        line.split(" ").forEach((word) => {
          ...
          w.textContent = clean + " ";
          reveal.appendChild(w); wordEls.push(w);
        });
        if (li < COPY.length - 1) { const br = document.createElement("span"); br.innerHTML = "<br><br>"; reveal.appendChild(br); }
      });
      const onS = () => {
        const trig = window.innerHeight * 0.6;
        wordEls.forEach((w) => { const r = w.getBoundingClientRect(); if (r.top < trig) w.classList.add("on"); else w.classList.remove("on"); });
      };
      window.addEventListener("scroll", onS, { passive: true });
      window.addEventListener("resize", onS, { passive: true });
      onS();
      cleanups.push(() => { window.removeEventListener("scroll", onS); window.removeEventListener("resize", onS); });

      const targets = "one credit passport that travels with you everywhere".split(" ");
      const ws = [].slice.call(reveal.querySelectorAll(".w"));
      const norm = (s) => (s || "").replace(/[^a-z]/gi, "").toLowerCase();
      let start = -1;
      for (let i = 0; i + targets.length <= ws.length; i++) {
        let ok = true;
        for (let j = 0; j < targets.length; j++) { if (norm(ws[i + j].textContent) !== norm(targets[j])) { ok = false; break; } }
        if (ok) { start = i; break; }
      }
      if (start >= 0) {
        const group = ws.slice(start, start + targets.length);
        const mark = document.createElement("mark"); mark.className = "hlmark";
        group[0].parentNode.insertBefore(mark, group[0]);
        group.forEach((w) => mark.appendChild(w));
        if (reduce) { mark.classList.add("lit"); }
        else {
          const oh = () => { if (mark.getBoundingClientRect().top < window.innerHeight * 0.56) { mark.classList.add("lit"); window.removeEventListener("scroll", oh); } };
          window.addEventListener("scroll", oh, { passive: true }); oh();
          cleanups.push(() => window.removeEventListener("scroll", oh));
        }
      }
    })();

    // ===== S1: showcase reveal + phone scale =====
    (function () {
      const showcase = $("showcase");
      if (showcase) {
        const io = new IntersectionObserver((es) => es.forEach((e) => { if (e.isIntersecting) showcase.classList.add("in"); }), { threshold: 0.2 });
        io.observe(showcase); cleanups.push(() => io.disconnect());
      }
      function scaleUI() {
        const wrap = document.querySelector(".phone .screenwrap"); const ui = $("ui");
        if (!wrap || !ui) return;
        const sw = wrap.clientWidth, sh = wrap.clientHeight; const scale = sw / 430;
        ui.style.transform = "scale(" + scale + ")"; ui.style.height = (sh / scale) + "px";
      }
      window.addEventListener("resize", scaleUI, { passive: true });
      window.addEventListener("load", scaleUI);
      const t = setTimeout(scaleUI, 60);
      if (document.fonts) document.fonts.ready.then(scaleUI);
      cleanups.push(() => { window.removeEventListener("resize", scaleUI); clearTimeout(t); });
    })();

    // ===== S1: canvas trailer loops + modal =====
    function drawLoop(canvas) {
      if (!canvas) return () => {};
      const ctx = canvas.getContext("2d"); const dpr = Math.min(window.devicePixelRatio || 1, 2); let raf = 0;
      ...
      resize(); const rz = () => resize(); window.addEventListener("resize", rz, { passive: true });
      let tt = 0;
      function frame() {
        const w = canvas.clientWidth, h = canvas.clientHeight; tt += 0.006; ctx.clearRect(0, 0, w, h);
        ctx.fillStyle = "#0b0c11"; ctx.fillRect(0, 0, w, h);
        const cx = w * (0.5 + 0.35 * Math.sin(tt)), cy = h * (0.5 + 0.3 * Math.cos(tt * 1.1));
        let g = ctx.createRadialGradient(cx, cy, 0, cx, cy, Math.max(w, h) * 0.7);
        g.addColorStop(0, "rgba(99,84,255,0.55)"); g.addColorStop(0.4, "rgba(75,59,255,0.18)"); g.addColorStop(1, "rgba(11,12,17,0)");
        ctx.fillStyle = g; ctx.fillRect(0, 0, w, h);
        const cx2 = w * (0.5 + 0.4 * Math.cos(tt * 0.8)), cy2 = h * (0.5 + 0.35 * Math.sin(tt * 0.9));
        let g2 = ctx.createRadialGradient(cx2, cy2, 0, cx2, cy2, Math.max(w, h) * 0.55);
        g2.addColorStop(0, "rgba(120,200,255,0.3)"); g2.addColorStop(1, "rgba(11,12,17,0)");
        ctx.fillStyle = g2; ctx.fillRect(0, 0, w, h);
        ctx.save(); ctx.translate(w / 2, h / 2); const s = Math.min(w, h) * 0.16;
        ctx.strokeStyle = "rgba(255,255,255,0.9)"; ctx.lineWidth = Math.max(2, s * 0.26); ctx.lineCap = "round"; ctx.lineJoin = "round";
        ctx.beginPath(); ctx.moveTo(-s, -s * 0.2); ctx.lineTo(-s * 0.1, s * 0.5); ctx.lineTo(s * 0.35, -s * 0.3); ctx.lineTo(s, s * 0.45); ctx.stroke(); ctx.restore();
    ...
    cleanups.push(drawLoop($("thumbLoop")));

    (function () {
      const modal = $("modal"); if (!modal) return;
      let mls = false; let stopModal = () => {};
      const open = () => { modal.classList.add("open"); document.body.style.overflow = "hidden"; if (!mls) { stopModal = drawLoop($("modalLoop")); mls = true; } };
      const close = () => { modal.classList.remove("open"); document.body.style.overflow = ""; };
      const tb = $("trailerBtn"), mc = $("modalClose"), mb = $("modalBackdrop");
      ...
      cleanups.push(() => {
        if (tb) tb.removeEventListener("click", open);
        if (mc) mc.removeEventListener("click", close);
        if (mb) mb.removeEventListener("click", close);
        document.removeEventListener("keydown", onKey);
        document.body.style.overflow = ""; stopModal();
      });
    })();

    // ===== WHO: marker underline + reveal =====
    (function () {
      const root = $("who"); if (!root) return;
      const hl = root.querySelector(".hl");
      const paths = hl ? hl.querySelectorAll(".hl-line path") : [];
      paths.forEach((p) => { const L = p.getTotalLength(); p.style.strokeDasharray = L; p.style.strokeDashoffset = reduce ? 0 : L; });
      const draw = () => paths.forEach((p) => { p.style.strokeDashoffset = 0; });
      if (reduce) { root.querySelectorAll(".reveal-up").forEach((el) => el.classList.add("in")); draw(); return; }
      ...
      root.querySelectorAll(".reveal-up").forEach((el) => io.observe(el)); cleanups.push(() => io.disconnect());
      if (hl) {
        const io2 = new IntersectionObserver((es) => es.forEach((e) => { if (e.isIntersecting) { setTimeout(draw, 260); io2.disconnect(); } }), { threshold: 0.9 });
        io2.observe(hl); cleanups.push(() => io2.disconnect());
      }
    })();

    // ===== ORBIT (showcase coins) =====
    (function () {
      const sc = $("showcase"); if (!sc) return;
      const orbs = [].slice.call(sc.querySelectorAll(".vorb"));
      orbs.forEach((o) => {
        const v = o.querySelector(".vcoin,.vtile"); if (v) v.style.setProperty("--sz", (o.dataset.sz || 60) + "px");
        const i = o.querySelector(".vorbI"); if (i) { i.style.setProperty("--dur", (o.dataset.dur || 9) + "s"); i.style.setProperty("--fy", o.dataset.fy || "-13px"); }
      });
      const pop = () => orbs.forEach((o, k) => { const t = setTimeout(() => o.classList.add("pop"), 280 + k * 74); cleanups.push(() => clearTimeout(t)); });
      if (reduce) { orbs.forEach((o) => o.classList.add("pop")); return; }
      const io = new IntersectionObserver((es) => es.forEach((e) => { if (e.isIntersecting) { pop(); io.disconnect(); } }), { threshold: 0.3 });
      io.observe(sc); cleanups.push(() => io.disconnect());
      if (window.matchMedia("(pointer:fine)").matches) {
        const pw = sc.querySelector(".phonewrap");
        const mm = (e) => {
          if (!pw) return;
          const r = pw.getBoundingClientRect();
          const cx = (e.clientX - (r.left + r.width / 2)) / r.width, cy = (e.clientY - (r.top + r.height / 2)) / r.height;
          orbs.forEach((o, k) => { const d = 8 + (k % 3) * 6; o.style.marginLeft = (cx * d) + "px"; o.style.marginTop = (cy * d) + "px"; });
        };
        window.addEventListener("mousemove", mm, { passive: true }); cleanups.push(() => window.removeEventListener("mousemove", mm));
      }
    })();

    // ===== HOW: connector lines, gauge ring, count-up =====
    (function () {
      const root = $("how"); if (!root) return;
      root.querySelectorAll(".ln").forEach((l) => { const L = l.getTotalLength(); l.style.setProperty("--L", L); });
      const prog = root.querySelector(".gprog"); let C = 0;
      if (prog) { C = 2 * Math.PI * 74; prog.style.strokeDasharray = C; prog.style.strokeDashoffset = reduce ? C * 0.1 : C; }
      function countUp() {
        const el = $("gv"); if (!el) return; const target = 810; let t0 = null; const dur = 1500;
        function step(ts) { if (!t0) t0 = ts; const p = Math.min((ts - t0) / dur, 1); const e = 1 - Math.pow(1 - p, 3); el.textContent = Math.round(target * e); if (p < 1) requestAnimationFrame(step); }
        requestAnimationFrame(step);
      }
      function go() {
        root.classList.add("go");
        root.querySelectorAll(".reveal-up").forEach((e) => e.classList.add("in"));
        if (prog) prog.style.strokeDashoffset = C * 0.1;
        if (reduce) { const el = $("gv"); if (el) el.textContent = "810"; } else countUp();
      }
      if (reduce) { go(); return; }
      const io = new IntersectionObserver((es) => es.forEach((e) => { if (e.isIntersecting) { go(); io.disconnect(); } }), { threshold: 0.3 });
      io.observe(root); cleanups.push(() => io.disconnect());
    })();

    // ===== TRUST: single-open accordion + reveal =====
    (function () {
      const root = $("trust"); if (!root) return;
      const rows = [].slice.call(root.querySelectorAll(".row")); const hds = [];
      rows.forEach((row) => {
        const hd = row.querySelector(".rowhd"); if (!hd) return;
        const handler = () => {
          const open = row.classList.contains("open");
          rows.forEach((r) => { r.classList.remove("open"); const b = r.querySelector(".rowhd"); if (b) b.setAttribute("aria-expanded", "false"); });
          if (!open) { row.classList.add("open"); hd.setAttribute("aria-expanded", "true"); }
        };
        hd.addEventListener("click", handler); hds.push([hd, handler]);
      });
      cleanups.push(() => hds.forEach(([h, fn]) => h.removeEventListener("click", fn)));
      if (reduce) { root.querySelectorAll(".reveal-up").forEach((e) => e.classList.add("in")); return; }
      ...
      root.querySelectorAll(".reveal-up").forEach((e) => io.observe(e)); cleanups.push(() => io.disconnect());
    })();

    // ===== CLOSE: reveal =====
    (function () {
      const root = $("close"); if (!root) return;
      if (reduce) { root.querySelectorAll(".ru").forEach((e) => e.classList.add("in")); return; }
      const io = new IntersectionObserver((es) => es.forEach((e) => { if (e.isIntersecting) { e.target.classList.add("in"); io.unobserve(e.target); } }), { threshold: 0.18 });
      root.querySelectorAll(".ru").forEach((e) => io.observe(e)); cleanups.push(() => io.disconnect());
    })();

    // ===== MARKET: trigger bento animations =====
    (function () {
      const root = $("market"); if (!root) return;
      if (reduce) { root.classList.add("go"); return; }
      const io = new IntersectionObserver((es) => es.forEach((e) => { if (e.isIntersecting) { root.classList.add("go"); io.disconnect(); } }), { threshold: 0.14 });
      io.observe(root); cleanups.push(() => io.disconnect());
    })();

    return () => { cleanups.forEach((fn) => { try { fn && fn(); } catch (e) {} }); };
  }, []);
  ...
  return (
    ...
    <nav className="nav">
      <div className="brand"><img id="brandLogo" alt="Blocrate" /></div>
  ...
  <main className="hero">
    ...
    <a className="pill" href="#"><span className="live"></span><span className="full">Now live — the credit layer for onchain lending</span><span className="short">Now live — the credit layer</span></a>
    ...
    <div className="cta-row">
      <a href="#" className="btn btn-primary">Request access</a>
      <a href="#" className="btn btn-ghost">Read the docs
...
<div id="s1">
  ...
  <section className="showcase" id="showcase">
    ...
    <div className="phonewrap">
      <div className="orbit"><div className="vorb" style={{left: "-30%", top: "5%"}} data-sz="72" data-dur="9.5" data-fy="-15px"><div className="vorbI"><div className="vcoin usdc" style={{"--sz": "72px"}}><span>USDC</span></div></div></div><div className="vorb" style={{left: "99%", top: "8%"}} data-sz="68" data-dur="8.8" data-fy="13px"><div className="vorbI"><div className="vcoin dai" style={{"--sz": "68px"}}><span>DAI</span></div></div></div><div className="vorb" style={{left: "-23%", top: "30%"}} data-sz="60" data-dur="10.2" data-fy="12px"><div className="vorbI"><div className="vtile" style={{"--sz": "60px", "--gc": "#8a7bff"}}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="4.5" y="4.5" width="15" height="15" rx="4.5"/><circle cx="12" cy="12" r="3.4"/></svg></div></div></div><div className="vorb" style={{left: "103%", top: "33%"}} data-sz="62" data-dur="9.0" data-fy="-13px"><div className="vorbI"><div className="vtile" style={{"--sz": "62px", "--gc": "#1aae8f"}}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="4.5" y="4.5" width="15" height="15" rx="4.5"/><circle cx="12" cy="12" r="3.4"/></svg></div></div></div><div className="vorb" style={{left: "-32%", top: "57%"}} data-sz="54" data-dur="10.6" data-fy="11px"><div className="vorbI"><div className="vtile" style={{"--sz": "54px", "--gc": "#2f6bff"}}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="4.5" y="4.5" width="15" height="15" rx="4.5"/><circle cx="12" cy="12" r="3.4"/></svg></div></div></div><div className="vorb" style={{left: "91%", top: "60%"}} data-sz="56" data-dur="9.4" data-fy="-12px"><div className="vorbI"><div className="vtile" style={{"--sz": "56px", "--gc": "#e08a2a"}}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="4.5" y="4.5" width="15" height="15" rx="4.5"/><circle cx="12" cy="12" r="3.4"/></svg></div></div></div><div className="vorb" style={{left: "-15%", top: "83%"}} data-sz="62" data-dur="9.8" data-fy="-13px"><div className="vorbI"><div className="vcoin usdt" style={{"--sz": "62px"}}><span>USDT</span></div></div></div><div className="vorb" style={{left: "101%", top: "86%"}} data-sz="56" data-dur="11" data-fy="10px"><div className="vorbI"><div className="vtile" style={{"--sz": "56px", "--gc": "#d4537e"}}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="4.5" y="4.5" width="15" height="15" rx="4.5"/><circle cx="12" cy="12" r="3.4"/></svg></div></div></div><div className="vorb" style={{left: "58%", top: "-7%"}} data-sz="52" data-dur="10.8" data-fy="10px"><div className="vorbI"><div className="vtile" style={{"--sz": "52px", "--gc": "#16b1a8"}}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="4.5" y="4.5" width="15" height="15" rx="4.5"/><circle cx="12" cy="12" r="3.4"/></svg></div></div></div></div>
...
<div className="status">
  ...
  <div className="modal" id="modal" role="dialog" aria-modal="true">
    ...
    <div className="panel">
      <canvas className="ploop" id="modalLoop"></canvas>
      ...
      <div className="pinner">
        <img className="pwm" id="modalwm" alt="Blocrate" />
        <div className="ptitle">The trailer is coming soon.</div>
        <div className="psub">We're putting the finishing touches on a 60-second film about portable onchain credit. Join the waitlist and we'll send it to you first.</div>
        <span className="pbadge"><span style={{width: "6px", height: "6px", borderRadius: "50%", background: "#8b7dff", display: "inline-block"}}></span>In production</span>
      </div>
    </div>
  </div>
    ...
    <div className="cards">
      ...
      <article className="card dark reveal-up d1">
        ...
        <div className="viz">
          ...
          <div className="gnum"><span className="v" id="gv">0</span><span className="m">/900</span><span className="t">Prime tier</span></div>
        </div>
  ...
  <div className="wrap">
    ...
    <div className="cols">
      ...
      <div className="col dark reveal-up d2">
        ...
        <div className="cfoot"><a href="#" className="wbtn ghost-d">Read the docs <svg className="warr" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg></a></div>
      </div>
      ...
      <div className="col light reveal-up d3">
        ...
        <div className="cfoot"><a href="#" className="wbtn primary">Get your passport <svg className="warr" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg></a></div>
      </div>
      ...
      <article className="cell wide d1">
        <div className="viz v-bnpl">
          ...
          <div className="pp">
            ...
            <div className="ppin">
              <div className="row" style={{alignItems: "flex-start"}}><span className="wm"><i></i>Blocrate</span><span className="chip"></span></div>
      ...
      <article className="cell wide d4">
        <div className="viz v-tradfi">
          ...
          <div className="pp seal">
            ...
            <div className="ppin" style={{padding: "14% 14%"}}><div className="row" style={{alignItems: "flex-start"}}><span className="wm" style={{fontSize: "10px"}}><i style={{width: "7px", height: "7px"}}></i></span></div><div className="row"><span className="ttl" style={{fontSize: "6.5px"}}>On-chain</span><div className="scn" style={{fontSize: "13px"}}>810</div></div></div>
          </div>
        </div>
    ...
    <div className="left">
      ...
      <div className="cardwrap reveal-up d3">
        ...
        <div className="card">
          ...
          <div className="pad">
            <div className="top">
              <div className="tbk"><span className="wm"><i></i>Blocrate</span><span className="tg2">Credit Passport</span></div>
              <div className="chip"></div>
            </div>
            <div className="bot">
  ...
  <section className="cta">
    ...
    <div className="inner">
      ...
      <div className="row ru d3">
        <a href="#" className="cbtn cbtn-primary">Request access</a>
        <a href="#" className="cbtn cbtn-ghost">Read the docs
    ...
    <div className="fwrap">
      <div className="ftop">
        <div className="cbrand">
          <span className="wm"><i></i>Blocrate</span>
          <p className="tl">The credit bureau for the stablecoin economy. One portable score, read from every protocol.</p>
          <div className="soc">
            <a href="#" aria-label="X"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M18.9 3H21l-6.5 7.4L22 21h-5.9l-4.6-6-5.3 6H3l7-8L2.5 3h6l4.2 5.5L18.9 3Zm-2.1 16h1.6L7.3 4.7H5.6L16.8 19Z"/></svg></a>
            <a href="#" aria-label="Farcaster"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 5h14M6 5v14M18 5v14M4 9h2M18 9h2M4 19h5M15 19h5M8 9h8v4"/></svg></a>
            <a href="#" aria-label="GitHub"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 2a10 10 0 0 0-3.16 19.49c.5.09.68-.22.68-.48v-1.7c-2.78.6-3.37-1.34-3.37-1.34-.46-1.16-1.11-1.47-1.11-1.47-.91-.62.07-.6.07-.6 1 .07 1.53 1.03 1.53 1.03.9 1.53 2.36 1.09 2.94.83.09-.65.35-1.09.63-1.34-2.22-.25-4.55-1.11-4.55-4.94 0-1.09.39-1.98 1.03-2.68-.1-.25-.45-1.27.1-2.65 0 0 .84-.27 2.75 1.02a9.6 9.6 0 0 1 5 0c1.91-1.29 2.75-1.02 2.75-1.02.55 1.38.2 2.4.1 2.65.64.7 1.03 1.59 1.03 2.68 0 3.84-2.34 4.69-4.57 4.94.36.31.68.92.68 1.85v2.74c0 .27.18.58.69.48A10 10 0 0 0 12 2Z"/></svg></a>
          </div>
        </div>
        <div className="col">
          <h4>Product</h4>
          <a href="#">Credit Passport</a><a href="#">For protocols</a><a href="#">For borrowers</a><a href="#">Pricing</a>
        </div>
        <div className="col">
          <h4>Developers</h4>
          <a href="#">Docs</a><a href="#">API reference</a><a href="#">Status</a><a href="#">Changelog</a>
        </div>
        <div className="col">
          <h4>Company</h4>
          <a href="#">About</a><a href="#">Careers</a><a href="#">Blog</a><a href="#">Contact</a>
        </div>
      </div>
      <div className="fbot">
